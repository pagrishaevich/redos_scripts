...
- **mount-manager.sh**: для записей CIFS в `/etc/fstab` добавлены `_netdev`, `nofail`, `x-systemd.automount`, `x-systemd.requires=network-online.target` и `x-systemd.after=network-online.target`, чтобы сетевые шары поднимались после перезагрузки при наличии сети, а не требовали ручной загрузки из конфига.
- **mount-manager.sh**: список записей `/etc/fstab` теперь распознаёт CIFS-строки с ведущими пробелами и не путает их с закомментированными строками.

### v1.6 (2026-05-12)

- **mount-manager.sh**: перед монтированием проверяет, не занята ли точка mount-point. Если директория уже смонтирована, скрипт показывает текущий источник и предлагает размонтировать её перед повторным монтированием, чтобы не получать `mount error(16): Device or resource busy`.
- **mount-manager.sh**: после добавления, обновления или удаления CIFS-записей в `/etc/fstab` выполняет `systemctl daemon-reload`, чтобы systemd не использовал старую версию mount-unit.

### v1.5 (2026-05-12)

- **mount-manager.sh**: если в Kerberos-режиме указан IP-адрес SMB-сервера и обратное DNS-разрешение не найдено, скрипт теперь запрашивает DNS-имя/FQDN сервера и продолжает монтирование вместо немедленного завершения.
...
- для автоподключения после перезагрузки добавляет в `/etc/fstab` systemd-automount и ожидание `network-online.target`;
- после изменения `/etc/fstab` обновляет состояние systemd через `systemctl daemon-reload`;
- перед монтированием проверяет, не занята ли точка mount-point, и предлагает размонтировать старое подключение;
...
5. Для Kerberos используйте DNS-имя или FQDN SMB-сервера, а не IP-адрес. Если введён IP, скрипт попробует найти имя через `getent hosts`; если имя не найдено, он запросит DNS-имя/FQDN вручную.
